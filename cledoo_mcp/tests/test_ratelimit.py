...
from odoo.addons.cledoo_mcp.lib import ratelimit
...
class TestRateLimitHttp(HttpCase):
    ...
    def setUpClass(cls):
        ...
        icp.set_param("cledoo_mcp.enabled", "True")
        icp.set_param("cledoo_mcp.rate_limit", "3")
    ...
    def test_register_endpoint_is_throttled_per_ip(self):
        self.env["ir.config_parameter"].sudo().set_param(
            "cledoo_mcp.register_rate_limit", "2")
        self.env.flush_all()
    ...
    def test_register_throttle_is_per_forwarded_ip(self):
        ...
        self.env["ir.config_parameter"].sudo().set_param(
            "cledoo_mcp.register_rate_limit", "1")
        self.env.flush_all()
        body = json.dumps({"client_name": "T",
                           "redirect_uris": ["https://claude.ai/cb"]})

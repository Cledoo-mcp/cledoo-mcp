...
from odoo.addons.cledoo_mcp.models import oauth as O
...
class TestAccessTtl(TransactionCase):
    def _set(self, val):
        self.env["ir.config_parameter"].sudo().set_param(
            "cledoo_mcp.oauth_access_ttl_hours", val)

    def test_default_is_eight_hours(self):
        self.env["ir.config_parameter"].sudo().search(
            [("key", "=", "cledoo_mcp.oauth_access_ttl_hours")]).unlink()
...
class TestTokenEndpointExpiresIn(HttpCase):
    def test_expires_in_matches_configured_ttl(self):
        ...
        self.env["ir.config_parameter"].sudo().set_param(
            "cledoo_mcp.oauth_access_ttl_hours", "2")

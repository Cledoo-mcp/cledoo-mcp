...
# cledoo_mcp.trusted_redirect_hosts parameter (comma-separated).
...
def _redirect_host_trusted(redirect_uri):
    ...
    raw = request.env["ir.config_parameter"].sudo().get_param(
        "cledoo_mcp.trusted_redirect_hosts",
...
class McpOAuthController(http.Controller):
    ...
    def register(self, **kw):
        ...
        limit = get_int_param(
            request.env, "cledoo_mcp.register_rate_limit", 10)
    ...
    def authorize(self, **kw):
        ...
        if request.httprequest.method == "GET":
            ...
            response = request.render("cledoo_mcp.oauth_consent", {
    ...
    def _token(self, kw):
        from odoo.addons.cledoo_mcp.lib.oauth_tokens import verify_pkce
        from odoo.addons.cledoo_mcp.models.oauth import access_ttl
        if not _resource_allowed(kw.get("resource")):
            return _json({"error": "invalid_target",
                          "error_description":
                              "resource must be %s/mcp" % _base_url()},
